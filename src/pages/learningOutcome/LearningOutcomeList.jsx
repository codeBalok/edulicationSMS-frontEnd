...
const LearningOutcomeList = () => {
    ...
    const getClassRoom = async () => {
        const data = await api.fetchLearningOutcome();
        const model = parentExtractor(data.data[0]?.parent_type)
        setProgram(data.data?.learning_outcome)
            ...
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-blue-500">
                    <tr>
                        ...
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Learning Outcome</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Level</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Assessment Methods</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Performance Criteria</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Mapping</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {program?.map((data, index ) => (
                        <tr key={data.id}>
                            ...
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.level}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.assessment_methods}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.performance_criteria}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.mapping}</td>
