...
const GradingList = () => {
    ...
    const getClassRoom = async () => {
        const data = await api.fetchGrading();
        const model = parentExtractor(data.data[0]?.parent_type)
        setProgram(data.data?.grading)
            ...
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-blue-500">
                    <tr>
                        ...
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Grade</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Points</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Total Marks</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Grade Range</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Min Mark</th>
                        <th className="px-4 py-2 text-left text-sm font-medium text-white">Max Mark</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {program?.map((data, index ) => (
                        <tr key={data.id}>
                            ...
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.grade_letter}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.grade_value}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.grade_points}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.grade_range}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.min_marks}</td>
                            <td className="px-4 py-2 text-sm text-gray-700">{data?.max_marks}</td>
